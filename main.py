...
#lihat semua list tutor (registered, verified ,unverified)
@app.get('/listtutor')
async def read_tutor(current_user: User = Depends(get_current_active_user)):
    return data

#akses data 1 calon tutor
...
#vstatus tutor: verified (status tutor yang lolos seleksi)       
@app.put('/verifytutor/{tutorid}')
async def update_status(tutor_id: int, current_user: User = Depends(get_current_active_user)):
    for list_tutor in data['Data Tutor']:
        if list_tutor['tutorid'] == tutor_id:
            list_tutor['status'] = 'verified'
        readfile.close()    

        with open("tutor.json", "w") as writefile:
            json.dump(data, writefile, indent=4)
        writefile.close()
    return {'message': 'Data changed successfully'}

#status tutor: unverified (status tutor yang tidak lolos seleksi)
@app.put('/unverifytutor/{tutorid}')
async def update_status(tutor_id: int, current_user: User = Depends(get_current_active_user)):
    for list_tutor in data['Data Tutor']:
        if list_tutor['tutorid'] == tutor_id:
            list_tutor['status'] = 'unverified'
        readfile.close()    
...
#hapus data tutor
@app.delete('/deletetutor/{tutorid}')
async def delete_tutor(tutor_id: int, current_user: User = Depends(get_current_active_user)):
    for list_tutor in data['Data Tutor']:
        if list_tutor['tutorid'] == tutor_id:
            data['Data Tutor'].remove(list_tutor)
        readfile.close()    
